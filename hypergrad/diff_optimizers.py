...
class DifferentiableOptimizer2:
    def __init__(self, loss_f, dim_mult, data_or_iter=None):
        """
        Args:
            loss_f: callable with signature (params, hparams, [data optional]) -> loss tensor
    ...
    def step(self, loss, params, hparams, create_graph,only_grad=False):
        raise NotImplementedError

    def __call__(self, loss,params, hparams, create_graph=True,only_grad=False):
        with torch.enable_grad():
            return self.step(loss,params, hparams, create_graph,only_grad=only_grad)
...
class GradientDescent2(DifferentiableOptimizer2):
    def __init__(self, loss_f, step_size, data_or_iter=None):
        super(GradientDescent2, self).__init__(loss_f, dim_mult=1, data_or_iter=data_or_iter)
        self.step_size_f = step_size if callable(step_size) else lambda x: step_size
        self.loss_lower = loss_f
    def step(self, loss, params, hparams, create_graph,only_grad=False):
