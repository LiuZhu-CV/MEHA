                ...
                with torch.no_grad():
                    with torch.no_grad():
                        ...
                        if len(xlist) > 1:
                            print(loss_L2(xgrad))
            if len(xlist)>1:
                if np.linalg.norm(Flist[-1]-Flist[-2])/np.linalg.norm(Flist[-2])<2e-2:#timelist[-1]>30 or args.element_wise:# or args.element_wise:#timelist[-1]>20loss_L2(xgrad)<1e-8:#loss_L2(xgrad)<1e-4:#timelist[-1]>6:#loss_L2(xgrad)<1e-8:#np.linalg.norm(xlist[-1]-xlist[-2])/np.linalg.norm(xlist[-2])<1e-8:
                    print(
                        'x_itr={},xdist={:.6f},xgrad={:.6f}, total_hyper_time={:.6f}, total_time={:.6f}'.format(
                            x_itr, torch.norm((x() - xstar) / xstar).detach().cpu().numpy(),
                            xgradlist[-1][0][0],
